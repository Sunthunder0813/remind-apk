...
| **Auto-Clearing Reminders** | **New.** Once a reminder's scheduled time passes, `reminderAt` is automatically cleared from the note — the pill/bell in the editor disappears, and the alarm icon on note cards/calendar rows goes away — **without removing or suppressing the actual device notification**, which still fires normally at the scheduled time. Clearing happens at two layers: (1) `DatabaseService._sweepExpiredReminders()` runs on every note read (`getAllNotes()`, `getNoteById()`, `getNoteByIdFresh()`), so any screen that reloads picks it up for free; (2) each of the three screens that display a reminder live (`NoteEditorScreen`, `NotesScreen`, `CalendarScreen`) also runs its own self-re-arming `Timer` that fires at the exact moment the soonest visible reminder expires, so the UI updates instantly even if the user is just sitting on that screen with nothing else triggering a reload. See **Auto-Clearing Reminders** section below for the full mechanism | ✅ Done |
| **Tap-to-Open Notifications** | **New.** Tapping a reminder notification opens the app and navigates straight to that note's editor (`NoteEditorScreen`), in both the warm-tap case (app already running) and the cold-start case (app was fully closed and the tap launched it). See **Tap-to-Open Notifications** section below for the full mechanism | ✅ Done |
...
| Local Notifications | Reminders fire as device notifications. **Tapping one now opens the app directly to that note** (see Tap-to-Open Notifications below) | ✅ Done |
...
| Home Screen Widget | Android-only "Today's Todos" widget, built via the `home_widget` package (v0.7.0+1). Renders **grouped, interactive rows**: each Calendar-tab note due today gets a header row (small custom checklist-icon glyph + title + time, if set), then either tappable checkbox rows (one per checklist item) or a single non-interactive plain-text **detail row** (for a note with real body content but no checklist), or nothing at all (a bare title-only note gets no sub-row), followed by a thin separator row between notes. Tapping a checkbox row toggles it straight through to the real Hive data and reflects back into the app instantly if it's open. See "Home Screen Widget" below for the full architecture, row types, and the cross-isolate caching bug that was the main blocker | ✅ Done (interactive checkboxes + non-interactive detail rows working; see Known Issues for one remaining native-layout regression) |

## Auto-Clearing Reminders

**Goal:** once a reminder's scheduled time has passed, the in-app UI (pill, bell icon, alarm badge) should stop showing it as "set" — without touching the actual device notification, which still needs to fire normally at the scheduled time regardless of what the note's own `reminderAt` field says afterward.

### Two layers

**1. Database-level sweep (`database_service.dart`)** — the source of truth. `_sweepExpiredReminders(List<Note> notes)` checks every note passed to it; if `reminderAt != null` and it's in the past, the field is set to `null` and written straight back to the box via `_notesBox.put(...)` (not the public `saveNote()`, to avoid a recursive call back into `getAllNotes()` via `WidgetService.refreshWidget()`). This is called from inside `getAllNotes()`, `getNoteById()`, and `getNoteByIdFresh()`, so **any** code path that reads a note through `DatabaseService` gets an already-correct `reminderAt` for free — this is what makes the app-was-fully-closed case work with zero extra wiring.

**2. Live re-arming timers (three screens)** — needed because the database sweep only runs on a *read*; if a screen is just sitting open with nothing triggering a reload, the sweep never fires on its own. Each of the three screens that display reminder state keeps a single `Timer?` that:
- scans whatever notes/state it currently has loaded for the soonest still-future `reminderAt`
- arms a one-shot `Timer` for exactly that remaining duration (capped at 1 minute, so a reminder days away doesn't sit on one giant uninterruptible timer)
- on fire, triggers a fresh read (which is what actually invokes the sweep above) and re-arms itself for whatever the next-soonest reminder now is

This re-arm-to-the-exact-moment approach (rather than a fixed `Timer.periodic` poll) means the UI updates the instant a reminder expires, with no polling overhead for far-future reminders.

| Screen | Timer field | Re-arm trigger | What firing does |
|---|---|---|---|
| `note_editor_screen.dart` | `_reminderExpiryTimer` | `initState`, picking a new reminder (`_pickReminder`/`_applySuggestedReminder`) | Sets `_reminderAt = null` directly (single note in scope) and triggers an auto-save |
| `notes_screen.dart` | `_reminderSweepTimer` | Every `_loadData()` call (including the one the timer itself causes) | Calls `_loadData()` again, which re-reads via `DatabaseService.getAllNotes()` (sweep runs there) |
| `calendar_screen.dart` | `_reminderSweepTimer` | Every `_loadNotes()` call (including the one the timer itself causes) | Calls `_loadNotes()` again, same pattern as above |

`NotesScreen` and `CalendarScreen`'s versions are near-identical (`_scheduleReminderSweep()` scans the loaded list for the soonest future `reminderAt` and re-arms); `NoteEditorScreen`'s is scoped to its single note (`_scheduleReminderExpiryCheck()`) and also handles the case where the time has *already* passed by the time the screen opens (fires immediately rather than scheduling).

### What this does NOT do

- Does not cancel or suppress the scheduled `flutter_local_notifications` notification — that's entirely independent, scheduled once via `scheduleNoteReminder()` and left alone.
- Does not delete the note or any of its other data — only the `reminderAt` field is cleared.
- Does not retroactively "snooze" or reschedule anything — once cleared, the user has to set a new reminder manually if they want one.

## Tap-to-Open Notifications

**Goal:** tapping a reminder notification opens the app and navigates straight to that note's editor — whether the app was already running (warm tap) or fully closed (cold start, where the tap itself is what launches the process).

### Mechanism

1. **Every scheduled notification carries a `payload`** — `notification_service.dart`'s `scheduleNoteReminder()` passes `payload: noteId` into `zonedSchedule(...)`, so the plugin remembers which note a given notification belongs to.
2. **Warm tap** (app already running, foreground or background): `flutter_local_notifications`' `initialize(...)` is given an `onDidReceiveNotificationResponse` callback. When the user taps, this fires with the `NotificationResponse`, and its `.payload` (the `noteId`) is pushed onto a new broadcast stream, `NotificationService.instance.noteTapStream`. `main.dart` subscribes to this stream once, for the app's whole lifetime, and on every event looks the note up via `DatabaseService.instance.getNoteById(noteId)` and pushes `NoteEditorScreen` for it.
3. **Cold start** (app process wasn't running; this launch was caused by the tap): the plugin's tap callback can't fire for this case — nothing was listening yet when the OS-level tap happened. Instead, `NotificationService.getLaunchNoteId()` calls the plugin's `getNotificationAppLaunchDetails()` right after `initialize()` completes, which tells you whether *this specific launch* was notification-caused and hands back the same payload. `main.dart` calls this once during startup, then — after `runApp()` and via `WidgetsBinding.instance.addPostFrameCallback` (since the global `navigatorKey`'s `NavigatorState` isn't available until the widget tree has painted its first frame) — pushes the same note editor route.

Both paths converge on one shared helper, `_openNoteFromNotification(noteId)` in `main.dart`, so the actual navigation logic only exists once.

### Why a global `navigatorKey`

Both the warm-tap stream listener and the cold-start path need to push a route from `main.dart`, outside of any widget's `build()` — there's no `BuildContext` available at either call site. `MaterialApp(navigatorKey: navigatorKey, ...)` (declared as a top-level `final GlobalKey<NavigatorState> navigatorKey = GlobalKey<NavigatorState>();` in `main.dart`) exposes `navigatorKey.currentState`, which has a `.push(...)` method that works the same as `Navigator.of(context).push(...)` — without needing a context at all.

### Known limitation

`noteId.hashCode.abs()` is used (unchanged, pre-existing) as the integer notification id `flutter_local_notifications` requires. `hashCode` is not guaranteed collision-free across different strings, so in a rare case two different notes could theoretically map to the same notification id, with one's scheduled reminder silently overwriting the other's. Not yet hit in practice; flagged here as a known theoretical gap, not fixed in this pass.
...
- `type: "item"` — one per checklist item (`noteId`, `itemId`, `text`, `done`). **Tappable** — renders with a checkbox icon and wires `setOnClickFillInIntent` so tapping toggles it through to Hive.
- `type: "detail"` — **new.** Used when a note has **no** checklist items but does have real body content (`note.content.trim().isNotEmpty`). Renders as plain text only — **no checkbox, no click intent, not tappable** — since this is just the note's free-text content being shown as a sub-line under its title, not an actual task. Distinct from `"item"` specifically so the native side never accidentally wires click/toggle behavior onto plain content.
- A note with **neither** checklist items **nor** any real content (e.g. a bare title-only "Untitled" note) gets **no sub-row at all** — just its header, nothing underneath. (Earlier behavior synthesized a fake checkbox row using the note's own title as fallback text, which produced a confusing duplicate "Untitled" header + "Untitled" checkbox; this was removed.)
- `type: "separator"` — inserted between notes, not after the last one.

`TodoRemoteViewsFactory.getViewAt()` switches on `row.type` and inflates `todo_widget_header_row.xml`, `todo_widget_row.xml`, `todo_widget_detail_row.xml`, or `todo_widget_separator_row.xml` accordingly (`getViewTypeCount()` is **`4`** to match — bumped from 3 when the detail row type was added; required any time a new distinct layout type is introduced, or the ListView's view recycling can misbehave). The checkbox icon (on `"item"` rows only) uses custom vector drawables (`todo_checkbox_checked.xml` / `todo_checkbox_unchecked.xml`) rather than `android.R.drawable.checkbox_on/off_background` — those stock drawables are legacy "background layer" assets meant to sit behind a real `CheckBox` widget's separate foreground glyph; used alone in a plain `ImageView` they render as invisible/blank on most themes, which was the cause of an earlier bug where tapping the widget checkbox visibly did nothing even though the underlying `done` value was flipping correctly in Hive.
...
### Header icon & row spacing

The widget header previously used a raw `📅` emoji prefix on the "Today's Todos" title text — replaced with a custom vector drawable, `ic_widget_header.xml` (a clipboard-with-checkmark glyph, tinted to the app's purple accent `#E3A2EE`), rendered via a small `ImageView` next to the title `TextView` instead of relying on emoji glyph rendering (which varies by OS/launcher font). Row padding was also increased across `todo_widget_row.xml`, `todo_widget_header_row.xml`, and `todo_widget_detail_row.xml` for better visual breathing room between stacked notes.
...
- `android/app/src/main/kotlin/com/example/remind/TodoRemoteViewsService.kt` — `RemoteViewsService` + `RemoteViewsFactory`; parses `todo_rows` JSON each time `onDataSetChanged()` runs, and inflates one of **four** row layouts per `getViewAt()` call based on `row.type` (`header` / `item` / `detail` / `separator`).
...
- `android/app/src/main/res/layout/todo_widget_layout.xml` — outer widget shell: header icon + text + divider + the `ListView` (`@+id/widget_list_view`) that the `RemoteViewsFactory` populates, plus an empty-state `TextView`.
- `android/app/src/main/res/layout/todo_widget_header_row.xml` — one note's header: bold title, time underneath (only visible if non-empty), task-count badge.
- `android/app/src/main/res/layout/todo_widget_row.xml` — one **tappable** checklist item row: checkbox `ImageView` + text, with strikethrough paint flags applied when `done`.
- `android/app/src/main/res/layout/todo_widget_detail_row.xml` — **new.** One non-interactive plain-text sub-line for a note's free-text content — no checkbox, no `row_root` id, no click wiring.
...
- `android/app/src/main/res/drawable/ic_widget_header.xml` — **new.** Custom vector drawable (clipboard + checkmark) replacing the previous raw emoji header icon.
...
- `lib/services/widget_service.dart` — `refreshWidget(List<Note> allNotes)` builds the grouped header/item/**detail**/separator row list and pushes it via `HomeWidget.saveWidgetData`/`updateWidget`; `applyPendingToggle()` reads the pending signal, flips the right `ChecklistItem`/`Note.isCompleted` in Hive, and notifies the main isolate via `IsolateNameServer`; `widgetBackgroundCallback` is the `@pragma('vm:entry-point')` top-level entry Dart wakes into.
- `lib/services/database_service.dart` — `getNoteByIdFresh(id)` (close+reopen box, cross-isolate-safe read) alongside the existing cached `getNoteById(id)`. Also now runs `_sweepExpiredReminders()` on every note read — see **Auto-Clearing Reminders** above.
- `lib/screens/note_editor_screen.dart` — `_syncChecklistFromHive()` (reads via `getNoteByIdFresh`, not the old date-filtered `todo_rows` JSON), wired to both `checklistUpdateStream` (push) and `didChangeAppLifecycleState` (resume, as a fallback). Also now owns `_reminderExpiryTimer` — see **Auto-Clearing Reminders** above.
- `lib/screens/calendar_screen.dart` — calls `refreshWidget()` after create/delete/toggle; `_swipeDeleteNote` also cancels the note's scheduled notification before deleting. Also now owns `_reminderSweepTimer` — see **Auto-Clearing Reminders** above.
...
- **Models:**
  - `Note` (`typeId: 0`) — `id, title, content, createdAt, updatedAt, reminderAt, categoryId, isArchived, isPinned, colorValue, backgroundImagePath, isCompleted, isCalendarReminder, calendarDate, checklistItems`.
    ...
    - `reminderAt` is now **auto-cleared** (set back to `null`) once its scheduled time passes — see **Auto-Clearing Reminders** above. This is a runtime behavior in `DatabaseService`, not a schema change; no new `@HiveField` was added for this.
...
├── main.dart                      # App entry point. Initializes Hive + notifications, populates AnimeLikeService's cache, pushes today's todos to the Android home screen widget once on cold start (`WidgetService.refreshWidget(...)`), registers the `checklistUpdateStream`/`IsolateNameServer` port the widget's background isolate pushes toggle notifications into, declares the top-level `navigatorKey` (`GlobalKey<NavigatorState>`, passed into `MaterialApp`) used for context-free navigation, subscribes to `NotificationService.instance.noteTapStream` (warm notification taps) and checks `getLaunchNoteId()` (cold-start notification launches) — both routed through the shared `_openNoteFromNotification(noteId)` helper that pushes `NoteEditorScreen` for the tapped note — then wires theme + home screen.
...
│   ├── database_service.dart      # Hive DB singleton — CRUD for notes, categories, AND `SavedAnime`. `init()` registers ALL adapters before opening any box — `NoteAdapter`, `CategoryAdapter`, `HiveAnimeSourceAdapter`, `SavedAnimeAdapter`, `ChecklistItemAdapter` (registered before the `notes` box opens, since `Note` embeds `List<ChecklistItem>`). Exposes `getNoteById(id)` (cached, fast) and `getNoteByIdFresh(id)` (closes + reopens the `notes` box to force a real disk read, bypassing this isolate's in-memory cache — needed specifically for cross-isolate reads after the widget's background callback writes a change). Also exposes `getSavedAnime(listType)`, `saveSavedAnime()`, `deleteSavedAnime()`, `isSavedAnime()`, and `getSavedAnimeEntry()`. **`_sweepExpiredReminders()`** runs inside `getAllNotes()`, `getNoteById()`, and `getNoteByIdFresh()` — clears any note's `reminderAt` once its time has passed (see Auto-Clearing Reminders above); writes back via a direct `_notesBox.put(...)`, not the public `saveNote()`, to avoid recursing back into itself via `WidgetService.refreshWidget()`.

│   ├── notification_service.dart  # Schedules/cancels local notifications for note reminders, with `payload: noteId` attached to every scheduled notification. Exposes `noteTapStream` (a broadcast `StreamController<String>`, fed by `onDidReceiveNotificationResponse` for warm taps) and `getLaunchNoteId()` (wraps `getNotificationAppLaunchDetails()`, for the cold-start case) — see Tap-to-Open Notifications above. `init()`'s `_plugin.initialize(...)` call now passes the `onDidReceiveNotificationResponse` callback alongside the existing `InitializationSettings`.
...
│   └── TodoRemoteViewsService.kt  # `RemoteViewsService` + `RemoteViewsFactory` — parses the grouped `todo_rows` JSON on every `onDataSetChanged()`, and inflates one of **four** row layouts per `getViewAt()` based on `row.type` (`header` / `item` / `detail` / `separator`).
...
├── res/layout/todo_widget_layout.xml      # Outer widget shell — header icon + text, divider, the `ListView` the factory populates, empty-state text.

├── res/layout/todo_widget_header_row.xml  # One note's header row — title, time (if set), task-count badge.

├── res/layout/todo_widget_row.xml         # One **tappable** checklist item row — checkbox `ImageView` + text, strikethrough when done.

├── res/layout/todo_widget_detail_row.xml  # **New.** One non-interactive plain-text content sub-line — no checkbox, no click wiring.
...
├── res/drawable/ic_widget_header.xml      # **New.** Custom vector drawable (clipboard + checkmark) — replaces the previous raw emoji widget header icon.
...
- **Notifications:** `flutter_local_notifications` + `timezone`. Tap handling (warm + cold-start) routes back into the app via a global `navigatorKey` — see Tap-to-Open Notifications above.
...
- **Home screen widget:** `home_widget: ^0.7.0+1` — Android-only so far; pairs Dart-side `saveWidgetData`/`updateWidget`/background-isolate callbacks with native Kotlin (`AppWidgetProvider` + `BroadcastReceiver` + `RemoteViewsService` + XML layouts/drawables) for fully interactive, two-way-synced checklist rows, plus non-interactive detail rows for content-only notes.
...
- **`zonedSchedule(...)`'s `uiLocalNotificationDateInterpretation` parameter is `required` in the currently-pinned `flutter_local_notifications` version** — don't remove it; some online guidance/changelogs for *other* versions of this plugin describe it as removed in v18+, but that does not apply to whatever version this project's `pubspec.yaml`/`pubspec.lock` currently resolves to. Always check the actual method signature (hover/go-to-definition in your editor) before trusting a version-specific claim like that.
...
- **Adding a new distinct `RemoteViews` row/layout type requires bumping `getViewTypeCount()`** in the relevant `RemoteViewsFactory` (here, `TodoRemoteViewsFactory.kt`) to match the new total count of distinct layouts — this was missed once already when the `detail` row type was added (caught and fixed; `getViewTypeCount()` is now `4`). Forgetting this can cause subtle `ListView` recycling bugs where rows of different types get recycled into each other's view holders.
...
- `DatabaseService.instance` is the single source of truth for all Hive reads/writes — never open Hive boxes directly in screens. **Exception:** `getNoteByIdFresh()` itself closes/reopens the `notes` box internally, but this is encapsulated inside `DatabaseService` — callers still never touch a `Box` directly. **`_sweepExpiredReminders()` is internal/private to `DatabaseService`** — don't call it directly from a screen; it always runs automatically as part of the existing read methods (`getAllNotes`/`getNoteById`/`getNoteByIdFresh`).
...
- `NoteEditorScreen` has **two auto-save layers** (1.2s debounce + 10s periodic `Timer.periodic`), **plus** a third, independent re-arming `Timer` (`_reminderExpiryTimer`) purely for auto-clearing an expired reminder — don't conflate the three; they serve different purposes and are cancelled/re-armed independently of each other.
...
- **Home screen widget is functionally complete for its core scope** (grouped interactive checklist rows + non-interactive detail rows, two-way sync between widget and app). It is **not** "realtime" in the sense of reacting to the passage of time on its own (midnight rollover, a reminder's time arriving) — only to explicit actions (app save/delete/toggle, or a widget tap). This is unrelated to and unaffected by the in-app auto-clearing-reminder timers described above, which only govern what the app's own screens display, not the widget. See the Known Issues subsection under Home Screen Widget for the open separator-layout regression and the occasional triple-toggle-per-tap issue before assuming this feature needs no further work. Don't restart the architecture (signal-via-SharedPreferences + Dart background callback, native code never writes Hive directly) — it's deliberate and working; any further work should build on it, not replace it.
- **Three separate screens each own their own reminder-expiry `Timer`** (`note_editor_screen.dart`'s `_reminderExpiryTimer`, `notes_screen.dart`'s `_reminderSweepTimer`, `calendar_screen.dart`'s `_reminderSweepTimer`) — these are independent, screen-local, and each must be cancelled in that screen's own `dispose()`. They all ultimately rely on `DatabaseService._sweepExpiredReminders()` as the actual source of truth; the timers exist only to trigger a re-read at the right moment, not to contain any clearing logic themselves. If adding a fourth screen that displays `reminderAt`, follow the same pattern rather than inventing a new one.
- **Notification taps are wired through a global `navigatorKey`, not a `BuildContext`.** `main.dart`'s `_openNoteFromNotification(noteId)` is the single shared entry point for both the warm-tap stream and the cold-start launch-details check — don't duplicate this logic per-call-site. If a future feature needs to navigate from outside a widget's `build()` (e.g. a different kind of background callback), reuse the existing `navigatorKey` rather than introducing a second one.
