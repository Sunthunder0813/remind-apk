  ...
  class NotesScreenState extends State<NotesScreen> {
    ...
    Future<void> _swipeDeleteNote(Note note) async {
      ...
      // below (cancelReminder, deleteNote, refreshWidget) are slow enough
      // that context.mounted is false by the time they finish, causing the
      // toast to silently no-op if called after them.
      ...
      // Undo window has passed (toast auto-dismisses after 3s) — only now is
    ...
    Future<void> _swipeDeleteFolder(Category folder) async {
      ...
      setState(() {
        _categories.removeWhere((c) => c.id == folder.id);
        for (final child in children) {
          _categories.removeWhere((c) => c.id == child.id);
        }
      });
